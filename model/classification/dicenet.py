...
class CNNModel(nn.Module):
    def __init__(self, args):
        ...
        for i in range(reps_at_each_level[1]):
            if i == 0:
                level3.append(ShuffleDICEBlock(2 * out_channel_map[1], out_channel_map[2], width=width, height=height,kernel_s=args.ksize))
            else:
                level3.append(ShuffleDICEBlock(out_channel_map[2], out_channel_map[2], width=width, height=height,kernel_s=args.ksize))
        ...
        width = int(width / 2)
        height = int(height / 2)
        level4.append(StridedDICE(channel_in=out_channel_map[2], width=width, height=height,kernel_size=args.ksize))
        for i in range(reps_at_each_level[2]):
            if i == 0:
                level4.append(ShuffleDICEBlock(2 * out_channel_map[2], out_channel_map[3], width=width, height=height,kernel_s=args.ksize))
            else:
                level4.append(ShuffleDICEBlock(out_channel_map[3], out_channel_map[3], width=width, height=height,kernel_s=args.ksize))
        ...
        width = int(width / 2)
        height = int(height / 2)
        level5.append(StridedDICE(channel_in=out_channel_map[3], width=width, height=height,kernel_size=args.ksize))
        for i in range(reps_at_each_level[3]):
            if i == 0:
                level5.append(ShuffleDICEBlock(2 * out_channel_map[3], out_channel_map[4], width=width, height=height,kernel_s=args.ksize))
            else:
                level5.append(ShuffleDICEBlock(out_channel_map[4], out_channel_map[4], width=width, height=height,kernel_s=args.ksize))
