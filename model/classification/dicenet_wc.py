...
from nn_layers.dice_wc import DICE_WC, StridedDICE
...
class ShuffleDICEBlock(nn.Module):
    def __init__(self, inplanes, outplanes, height, width, c_tag=0.5, groups=2):
        ...
        self.layer_right = nn.Sequential(
            ...
            DICE_WC(channel_in=self.right_part_out, channel_out=self.right_part_out, height=height, width=width)
        )
...
if __name__ == '__main__':
    ...
    for scale in config_all.sc_ch_dict.keys():
        for size in [96]:
            ...
            model = CNNModel(args)
            input = torch.randn(1, 70, size, size)
