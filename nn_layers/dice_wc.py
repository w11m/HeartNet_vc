...
class DICE_WC(nn.Module):
    ...
    def forward(self, x):
        ...
        outputs = torch.cat((out_ch_wise, out_w_wise), 1)
...
class StridedDICE(nn.Module):
    ...
    def __init__(self, channel_in, height, width, kernel_size=3, dilation=[1,1,1], shuffle=True):
        ...
        self.right_layer =  nn.Sequential(
            ...
            DICE_WC(channel_in, channel_in, height, width, kernel_size=kernel_size, dilation=dilation,
                 shuffle=shuffle),
            CBR(channel_in, channel_in, 1, 1)
        )
...
if __name__ == '__main__':
    ...
    model = DICE_WC(channel_in, channel_out, width, height, shuffle=True)
    model.eval()

    input = torch.Tensor(bsz, channel_in, 96, 96).fill_(1)
    out = model(input)
